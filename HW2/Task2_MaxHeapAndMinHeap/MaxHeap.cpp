...
template <typename T>
void MaxHeap::insert(const T& data){
    //일단 값을 넣어
    this->last++;
    ...
    int dummy = this->last;
    while(curr){
        if(this->list[curr] < this->last[dummy]){
            int temp = this->list[curr];
            this->list[curr] = this->last[dummy];
            this->last[dummy] = temp;
        }
        dummy = curr;
...
template <typename T>
T MaxHeap::extract(){// same as delete
    //만약 비었다면 빈 값보내기
    if(isEmpty()){
        return T();
    }
...
template <typename T>
void MaxHeap::swap(T& dataA, T& dataB){
    if(dataA < dataB){
        T dummy = dataA;
        dataA = dataB;
        dataB = dummy;
...
template <typename T>
T MaxHeap::peek() const {// get the root item but not delete
    if(isEmpty()){
        return T();
    }else{
        return this->list[0];
